...
object PersistentActors extends App {
  ...
  case class Invoice(recipient: String, date: LocalDate, amount: Int)

  case class InvoiceBulk(invoices: List[Invoice])

  case object Shutdown

  case class InvoiceRecorded(id: Int, recipient: String, date: LocalDate, amount: Int)
    ...
    override def receiveCommand: Receive = {
      case Invoice(recipient, date, amount) =>
        ...
        /**
          ...
          * 2. you persist the event, the pass in the callback that will get triggered once the event is written
  ...
  for (i <- 1 to 10) {
    accountant ! Invoice("The company co.", LocalDate.now(), i * 1000)
  }

  /**
    * Persistence failures: onPersistFailure and onPersistRejected
    */

  /**
    * Persisting multiple events: persistAll
    */
  //  val invoices = for (i <- 1 to 5) yield Invoice("The awesome invoice", LocalDate.now(), i * 2000)
  //  accountant ! InvoiceBulk(invoices.toList)

  /**
    * never call persist ot persistall from futures!!!
    *
